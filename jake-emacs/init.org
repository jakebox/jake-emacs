...
* Things to do
+ Make jib/listify work on a selection
  1. Procedure:
  2. Check if there is an active area
     * If so, count lines in area, deselect, go to first line, run as usual for n times
     * If not, run normally
+ Get mw-thesaurus key
+ Improve LaTeX editing/compiling/viewing
+ Auto disable/enable hl-line-mode on describe-face
+ Consider bufler
+ Consider company-box
+ Classy org WIP
...
** Dependencies
Here is a list of programs I have installed for Emacs. They are not explicitly required, however.
...
* Files, variables, etc
** Loading custom functions/custom file
...
#+END_SRC
*** General Keybindings
Below are general keymaps for the various Evil modes.
#+BEGIN_SRC emacs-lisp :tangle yes
  ;; All-mode keymaps
  (general-def
